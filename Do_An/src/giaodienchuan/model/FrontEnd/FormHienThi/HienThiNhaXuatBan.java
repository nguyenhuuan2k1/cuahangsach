...
public class HienThiNhaXuatBan extends FormHienThi {
    ...
    public HienThiNhaXuatBan() {
        setLayout(new BorderLayout());

        mtb = new MyTable();
        mtb.setPreferredSize(new Dimension(1200 - 250, 600));
        mtb.setHeaders(new String[]{"STT", "Mã nhà xuất bản", "Tên nhà xuất bản", "Địa chỉ"});
        mtb.setColumnsWidth(new double[]{.5, 3, 4, 5});
        mtb.setAlignment(0, JLabel.CENTER);
        mtb.setupSort();
        setDataToTable(qlnxb.getDsnxb(), mtb);

        // ======== search panel ===========
        cbTypeSearch = new JComboBox<>(new String[]{"Tất cả", "Mã nhà xuất bản", "Tên nhà xuất bản", "Địa chỉ"});
    ...
    private void txSearchOnChange() {
        setDataToTable(qlnxb.search(txTim.getText(), cbTypeSearch.getSelectedItem().toString()), mtb);
    }

    private void setDataToTable(ArrayList<NhaXuatBan> data, MyTable table) {
        table.clear();
        int stt = 1; // lưu số thứ tự dòng hiện tại
        for (NhaXuatBan nxb : data) {
            table.addRow(new String[]{String.valueOf(stt), nxb.getMaNXB(), nxb.getTenNXB(), nxb.getDiaChi()});
            stt++;
        }
    }
}
