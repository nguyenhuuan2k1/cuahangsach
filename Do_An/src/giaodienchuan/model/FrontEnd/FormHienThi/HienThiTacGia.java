...
public class HienThiTacGia extends FormHienThi {
    ...
    public HienThiTacGia() {
        setLayout(new BorderLayout());

        mtb = new MyTable();
        mtb.setPreferredSize(new Dimension(1200 - 250, 600));
        mtb.setHeaders(new String[]{"STT", "Mã tác giả", "Tên tác giả", "Mô tả"});
        mtb.setColumnsWidth(new double[]{.5, 3, 4, 5});
        mtb.setAlignment(0, JLabel.CENTER);
        mtb.setupSort();
        setDataToTable(qltg.getDstg(), mtb);

        // ======== search panel ===========
        cbTypeSearch = new JComboBox<>(new String[]{"Tất cả", "Mã tác giả", "Tên tác giả", "Mô tả"});
    ...
    private void txSearchOnChange() {
        setDataToTable(qltg.search(txTim.getText(), cbTypeSearch.getSelectedItem().toString()), mtb);
    }

    private void setDataToTable(ArrayList<TacGia> data, MyTable table) {
        table.clear();
        int stt = 1; // lưu số thứ tự dòng hiện tại
        for (TacGia tg : data) {
            table.addRow(new String[]{String.valueOf(stt), tg.getMaTG(), tg.getTenTG(), tg.getMoTa()});
            stt++;
        }
    }
}
